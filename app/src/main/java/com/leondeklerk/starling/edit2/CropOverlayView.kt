...
@JvmOverloads constructor(context: Context?, attrs: AttributeSet? = null) : View(context, attrs) {
    ...
    private fun drawBackground(canvas: Canvas) {
        ...
        if (!isNonStraightAngleRotated || !VersionChecker.isAtLeastJ18()) {
            ...
            if (VersionChecker.isAtLeastO26()) canvas.clipOutPath(mPath)
            else canvas.clipPath(mPath)
    ...
    private fun drawGuidelines(canvas: Canvas) {
        if (mGuidelinePaint != null) {
            ...
            val x1: Float = rect.left + oneThirdCropWidth
            val x2: Float = rect.right - oneThirdCropWidth
            ...
            val y1: Float = rect.top + oneThirdCropHeight
            val y2: Float = rect.bottom - oneThirdCropHeight
    ...
    private fun onActionDown(x: Float, y: Float) {
        mMoveHandler =
            mCropWindowHandler.getMoveHandler(x, y, mTouchRadius, mCenterMoveEnabled)
    ...
    private fun callOnCropWindowChanged(inProgress: Boolean) {
        try {
            ...
            Timber.e(e, "Exception in crop window changed")
        }
    }
