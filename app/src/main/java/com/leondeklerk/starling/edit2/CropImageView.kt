...
class CropImageView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) :
            ...
            Glide.with(mImageView.context)
                ...
                .into(object : CustomTarget<Bitmap>() {
                    override fun onResourceReady(resource: Bitmap, transition: Transition<in Bitmap>?) {
                        setProgressBarVisibility()
                        mInitialDegreesRotated = 0
                        setBitmap(
                            resource,
                            0,
                            uri,
                            BitmapUtils.calculateInSampleSizeByMaxTextureSize(
                                resource.width,
                                resource.height
                            ),
                            1
    ...
    public override fun onRestoreInstanceState(state: Parcelable) {
        if (state is Bundle) {
            ...
            if (imageUri == null && mBitmap == null && mImageResource == 0) {
    ...
    private fun setProgressBarVisibility() {
        val visible = (
            mShowProgressBar && (mBitmap == null && mBitmapCroppingWorkerJob != null)
            )
